...
SELECT member, facility, cost FROM (
    SELECT mem.firstname || ' ' || mem.surname AS member,
       	   fac.name AS facility,
           CASE WHEN mem.memid = 0 THEN bk.slots*fac.guestcost
                ELSE bk.slots*fac.membercost END AS cost
      FROM Members mem
      JOIN Bookings bk
        ON mem.memid = bk.memid
      JOIN Facilities fac
...
SELECT name, revenue FROM (
       SELECT fac.name, 
              SUM(CASE WHEN memid = 0 THEN slots*fac.guestcost
                       ELSE slots*fac.membercost END) AS revenue
         FROM Bookings bk
         JOIN Facilities fac
